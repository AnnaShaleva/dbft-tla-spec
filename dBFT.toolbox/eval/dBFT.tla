...
RMSendChangeView(r) ==
  ...
  /\ GetNewView(rmState[r].view) <= MaxView \* restrict MaxView for good nodes to avoid TemporalBlockAcceptanceProperty violation (node must be able to accept ChangeView and to actually change view after ChangeView was sent) 
...
RMReceiveChangeView(r) ==
  ...
  /\ GetNewView(rmState[r].view) <= MaxView \* TODO: get rid of MaxView and set the model constraint.
...
Inv ==
  \/ Cardinality({r \in RM : (r \notin RMFault /\ rmState[r].type = "commitSent")}) /= 1
  \/ Cardinality({r \in RM : (r \notin RMFault /\ rmState[r].type /= "blockAccepted" /\ rmState[r].type /= "commitSent" /\ IsViewChanging(r))}) /= 2

\* Invariat for 4 nodes setup.
Inv2 ==
  \/ Cardinality({r \in RM : (r \notin RMFault /\ rmState[r].type = "commitSent")}) /= 1
  \/ LET rCommitted == CHOOSE r \in RM : r \notin RMFault /\ rmState[r].type = "commitSent"
     IN
        \/ Cardinality({r \in RM : (r \notin RMFault /\ rmState[r].type = "initialized" /\ (rmState[r].view = rmState[rCommitted].view + 1))}) /= 2
        \/ Cardinality({r \in RMFault : rmState[r].type = "commitSent" /\ rmState[r].view = rmState[rCommitted].view + 1}) /= 1
...
\* For every behaviour it's true that eventually (i.e. at least once through the behaviour)
\* block will be accepted. It is something that dBFT must guarantee (an in practice this
\* condition is violated).
Liveness == <>(\E r \in RM : rmState[r].type = "blockAccepted")

\* 
LivelockAndDeadlockFree == (\E r \in RM : rmState[r].type = "commitSent") ~> (\E r \in RM : rmState[r].type = "blockAccepted")


\* The complete specification of the protocol written as a temporal  formula.  
Spec == 
       \* This part specifies only what the system MAY do (i.e. the set of possible
       \* allowed behaviours for the system).E.g. this part allows
       \* the behaviour to end with an infinite set of stuttering steps and never
       \* reach the state where at least one node is committed or accepted the
       \* block. To forbid such behaviours we must specify what the system MUST
       \* do. It will be specified below with the help of liveness and fairness
       \* conditions.
       Init /\ [][Next]_<<msgs, rmState>>
       \* This part specifies what the system MUST do.
       /\ Liveness


CommitSent == \E r \in RM : RMSendCommit(r)
ChangeViewSentAndReceived == \E r \in RM : RMSendChangeView(r) \/ RMReceiveChangeView(r)
BlockAccepted == \E r \in RM : RMAcceptBlock(r)

\* This is an invariant that should be violated if there's a bug in spec.
\* "Once the node entered in the "commit" state, the "blockAccepted" state must be reached eventually.
\*AcceptBlockInv ==
FairSpec == Spec  /\  WF_vars(ChangeViewSentAndReceived) /\ SF_vars(BlockAccepted)

TemporalBlockAcceptanceProperty == [](<><<Cardinality({r \in RM : rmState[r].type = "blockAccepted"}) >= 1>>_<<>>)
...
\* Last modified Mon Dec 26 10:05:41 MSK 2022 by anna
